...
func (acs *AtomicContainerSig) VerifyAttestationSignature(secret *secrets.PgpSigningSecret, attestationHash string) error {
	hostStr, err := acs.Json()
	if err != nil {
		return err
	}
	return attestation.VerifyMessageAttestation(secret.PublicKey, attestationHash, hostStr)
}
